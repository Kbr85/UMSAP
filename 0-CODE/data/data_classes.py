...
class DataObjTarProtFile(MyModules):
	...
	def TarProt2CutEvolution(self, fileO):
		""" Calculates the relative time evolution of the cleavage per residue
		"""
		# print(self.pLength)
		# print(self.dataF)
		# print('')		
	 #--> Data frame columns
		colHeader = dmethods.ListColHeaderCutEvolutionFile(self.nExp)	
		# print('Column Header: ', colHeader)
		# print('')
	 #---
	 #--> Create empty data frames
		df_data = pd.DataFrame(columns=colHeader) 
		df_out  = pd.DataFrame(columns=colHeader) 
		# print('Empty dataframe:')
		# print(df_data)
		# print('')
	 #---
	 #--> Fill df_data
	  #--> Get rows as list of list
		rows = self.dataF.apply(
			self.TarProt2CutEvolutionHelper1, 
			raw  = True,
			axis = 1,
		).to_list()
	  #---
	  #--> Flat list and delete empty elements
		rows = itertools.chain(*rows)
		rows = [x for x in rows if x]
	  #---
	  #--> Add rows
		df_data = df_data.append(
			pd.DataFrame(rows, columns=colHeader), 
			ignore_index=True,
		)
	  #---
	 #---
	 #--> Get first non zero value
		noZero = df_data.iloc[:,1:].replace(0, np.nan).bfill(1).iloc[:, 0]
		# print(noZero)
		# print('')
	 #---
	 #--> Calculate ratios
		df_data.iloc[:,1:] = df_data.iloc[:,1:].div(noZero, axis=0)
		# print('Filled dataframe:')
		# print(df_data)
		# print('')
	 #---
	 #--> Get unique list of residue numbers
		resN = df_data['Residue Number'].unique()
		# print(resN)
	 #---
	 #--> Sum values for each residue and append to df_out
		sumDF = []
		for k in resN:
			b = df_data[df_data['Residue Number']==k]
			a = b.sum(axis=0)
			a['Residue Number'] = k
			sumDF.append(a)
		df_out = df_out.append(sumDF, ignore_index=True)
		# print('Output')
		# print(df_out)
		# print('')
	 #---
	...
	def TarProt2CutEvolutionHelper1(self, row):
		""" Fill df_data in TarProt2CutEvolution """
	 #--> Variables
		k       = False # Return empty string if peptide is not FP
		meanInt = []    # Hold average intensity for each experiment
	 #---
	 #--> Check peptide in row is FP and since your looping exp get meanInt
		for e in range(0, self.nExp, 1):
			val = row['Exp'+str(e+1)][0][0]
			if val == 1:
				k = True
				meanInt.append(mean(row['Exp'+str(e+1)][1]))			
			else:
				meanInt.append(0)
		if k:
			pass
	 ...
	 #--> Data for Nterm
		if row['Nterm'] - 1 <= 0:
			...
			a = [row['Nterm'] - 1, *meanInt]
	 #---
	 #--> Data for Cterm
		if row['Cterm'] == self.pLength[0]:
			b = []
		else:
			b = [row['Cterm'], *meanInt]
	 #---
